        ...
        if self.isLeech:
            ...
            if not files:
                ...
                for index, (link, name) in enumerate(files.items(), start=1):
                    fmsg += f"{index}. <a href='{link}'>{file_}</a>\n"
            ...
            auto = sendMessage(msg + msgt + fwdpm, self.bot, self.message)
            Thread(target=auto_delete, args=(self.bot, self.message, auto)).start()
...
qb_zip_leech_handler = CommandHandler(BotCommands.QbZipLeechCommand, qb_zip_leech,
                                filters=CustomFilters.authorized_chat | CustomFilters.authorized_user, run_async=True)
auto_leech_handler = MessageHandler(CustomFilters.mirror_torrent_and_magnets & Filters.chat_type, leech, run_async=True)
